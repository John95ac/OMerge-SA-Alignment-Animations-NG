...
struct SoundWithPlayback {
    ...
    SoundWithPlayback() : playback("once") {}
...
struct OrderedPluginData {
    ...
    SetPresetResult setPreset(const std::string& animationKey, const std::string& soundFile,
                              const std::string& playback = "once") {
...
// Normalizar playback a minúsculas - MODIFICADO para manejar vacío
std::string NormalizePlayback(const std::string& playback) {
    if (playback.empty()) {
        return "once";  // Si está vacío, default es "once"
    }

    std::string normalized = playback;
    std::transform(normalized.begin(), normalized.end(), normalized.begin(), ::tolower);

    // Validar que sea "loop" o "once"
    if (normalized != "loop" && normalized != "once") {
        return "once";  // valor por defecto
    }
...
ParsedRule ParseRuleLine(const std::string& key, const std::string& value) {
    ...
    if (parts.size() >= 2) {
        ...
        // Parsear el tercer parámetro (playback) - MEJORADO
        if (parts.size() >= 3) {
            std::string playbackStr = Trim(parts[2]);
            rule.playback = NormalizePlayback(playbackStr);
        } else {
            rule.playback = "once";  // valor por defecto si no existe
        }
    ...
    try {
        while (pos < len && iter++ < maxIters) {
            ...
            while (pos < len && soundIter++ < maxIters) {
                ...
                // Parsear segundo elemento (playback)
                std::string playback = "once";  // valor por defecto
